...
class Attention(nn.Module):
    ...
    #         (q, k, v),
    #     )
    #     # with torch.amp.autocast(device_type="cuda", dtype=torch.bfloat16): # liyi todo flashattn only support float16
    #     try:
    #         with sdpa_kernel(SDPBackend.FLASH_ATTENTION):
    #             out = F.scaled_dot_product_attention(q, k, v)
    #     except Exception as e:
    #         print(f"FlashAttention not supported, using fallback: {e}")
    #         assert False
...
class MultiviewTransformer(nn.Module):
    ...
    def forward(
        ...
        for block, mix_block in zip(self.transformer_blocks, self.time_mix_blocks):
            ...
            # x = deepspeed.checkpointing.checkpoint(block, x, context)
            ...
            # x_mix = deepspeed.checkpointing.checkpoint(mix_block, x, time_context, num_frames)
            x_mix = torch.utils.checkpoint.checkpoint(mix_block, x, time_context, num_frames, use_reentrant=False)
